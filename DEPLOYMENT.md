...
`php artisan backup:run` مجدول تلقائياً يومياً الساعة 3 صباحاً (عبر نفس `schedule:run` أعلاه) — ينشئ نسخة `mysqldump` من قاعدة البيانات + أرشيف `tar.gz` لملفات `storage/app`، ويحتفظ بآخر 7 نسخ فقط بـ `storage/app/backups` (حذف تلقائي للأقدم).

**هذا نسخ احتياطي محلي فقط** — لو السيرفر نفسه تعطّل أو انفقد قرصه، هذي النسخ بتضيع معه. يُنصح بشدة بإضافة نسخ خارجي دوري فوق هذا، مثلاً:

```bash
# مثال: نسخ يومي لآخر backup لسيرفر/تخزين خارجي عبر rsync
0 4 * * * rsync -az /path/to/CRM-KW/"LARAVEL CRM V2"/storage/app/backups/ user@backup-server:/backups/crm/
```

أو رفعها لتخزين سحابي (S3 أو مشابه) عبر `aws s3 sync` بنفس الفكرة.
