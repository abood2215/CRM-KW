...
/**
 * Turns raw campaign input (an explicit recipient array and/or a contact list)
 * into find-or-create Contact rows, then hands them to CampaignBlacklistFilter.
 * Extracted from the 130-line inline block that used to live in
 * CampaignController::store().
 */
class CampaignRecipientResolver
{
    public function __construct(private readonly CampaignBlacklistFilter $blacklistFilter)
    {
    }
     ...
     */
    public function resolve(array $recipientInput, ?int $contactListId, ?string $templateName, User $actor): array
    {
