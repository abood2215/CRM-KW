...
const AddContactModal = ({ open, onClose, defaultStage = 'new' }) => {
  ...
  useEffect(() => {
    if (open) setForm(emptyForm(defaultStage));
  }, [open, defaultStage]);

  const mutation = useMutation({
    mutationFn: (data) =>
      contacts.createContact({ ...data, budget: data.budget ? Number(data.budget) : undefined }),
    onSuccess: () => {
    ...
    onError: (e) => toast.error(e?.response?.data?.message || 'فشل إضافة جهة الاتصال'),
  });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          ...
          >
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-xl font-black text-slate-800">إضافة جهة اتصال جديدة</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-black text-slate-600 mb-1.5">مرحلة المتابعة</label>
                  <select
                    value={form.pipeline_stage} onChange={(e) => setForm((f) => ({ ...f, pipeline_stage: e.target.value }))}
                    className="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                  >
