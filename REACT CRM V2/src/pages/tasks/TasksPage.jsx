...
const TasksPage = () => {
  ...
  const { data: taskList = [], isLoading } = useQuery({
    ...
    queryFn: () => tasksApi.getTasks({ status: filter }).then((res) => res.tasks),
  });
            ...
            {isLoading ? (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <h4 className={cn('text-sm lg:text-base font-black', task.status === 'completed' ? 'text-slate-400 line-through' : 'text-slate-800')}>
                        {task.title}
                      </h4>
                      <span className={cn('text-[10px] font-black px-2 py-0.5 rounded-lg uppercase tracking-tight border flex-shrink-0', priorityColors[task.priority || 'low'])}>
                        {task.priority === 'high' ? 'عالي' : task.priority === 'medium' ? 'متوسط' : 'عادي'}
                      </span>
                    </div>
                    {task.description && <p className="text-xs lg:text-sm font-medium text-slate-400 mb-2 line-clamp-1">{task.description}</p>}
                      ...
                      {task.due_date && (
                        <div
                          className={cn(
                            ...
                            task.status !== 'completed' && isPast(new Date(task.due_date)) && !isToday(new Date(task.due_date)) ? 'text-rose-600' : 'text-slate-500'
                          )}
                        >
                          <Clock size={11} />
              ...
              <div className="bg-white p-12 lg:p-20 rounded-2xl border border-slate-100 shadow-sm text-center">
                ...
                <p className="text-slate-400 font-medium max-w-xs mx-auto mt-2 text-sm leading-relaxed">
                  {filter === 'pending' ? 'جميع المهام تم إنجازها! يمكنك البدء بمهام جديدة.' : 'لا توجد مهام مطابقة لتصفيتك.'}
                </p>
              </div>
            )}
          </div>
