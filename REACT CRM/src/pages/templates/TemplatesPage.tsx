...
const TemplatesPage: React.FC = () => {
  ...
  const syncByNumberMutation = useMutation({
    mutationFn: (numberId: number) => api.post(`/whatsapp-numbers/${numberId}/sync-templates`),
    onSuccess: () => {
      setTimeout(() => queryClient.invalidateQueries({ queryKey: ['templates'] }), 3000);
      toast.success('جاري المزامنة في الخلفية...');
    },
    onError: (e: any) => toast.error(e?.response?.data?.message || 'فشلت المزامنة.'),
  });
...
const TemplateFormModal: React.FC<{
  ...
  const [form, setForm] = useState<FormState>(() => template ? {
    name:           template.name,
    language:       template.language,
    category:       template.category as FormState['category'],
    status:         template.status   as FormState['status'],
    header_type:    (template.header_type === 'text' ? 'text' : 'none') as FormState['header_type'],
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select value={form.category} onChange={e => set('category', e.target.value as FormState['category'])} required
                className="w-full h-11 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                <option value="marketing">تسويقي</option>
                <option value="utility">خدمي</option>
                <option value="authentication">مصادقة</option>
