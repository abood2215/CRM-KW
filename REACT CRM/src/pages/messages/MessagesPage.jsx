...
const flag = (c) => [...c.toUpperCase()].map(l => String.fromCodePoint(l.charCodeAt(0) + 127397)).join('');
const COUNTRIES = [
...
const MessagesPage = () => {
  ...
  const [newMsgMode, setNewMsgMode]           = useState('template');
  ...
  const { data: approvedTemplates = [], isLoading: loadingTemplates, refetch: refetchTemplates } = useQuery({
  ...
  const syncTemplatesMutation = useMutation({
    mutationFn: async () => {
      ...
      const numbers = data.whatsapp_numbers ?? [];
      const cloud = numbers.find((n) => n.phone_number_id && n.status === 'connected');
    ...
    onError: (e) => toast.error(e?.message || 'فشلت المزامنة'),
  });

  const sendTemplateMutation = useMutation({
    mutationFn: (p) => api.post(`/conversations/${selectedId}/send-template`, p),
    onSuccess: () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (status) => api.put(`/conversations/${selectedId}/status`, { status }),
    onSuccess: (_, status) => {
      queryClient.invalidateQueries({ queryKey: ['conversations'] });
  ...
  useEffect(() => {
    ...
    channel.listen('.MessageStatusUpdatedEvent', (e) => {
      const curId = selectedIdRef.current;
      if (e.conversation_id === curId) {
        queryClient.setQueryData(['messages', curId], (old = []) =>
          old.map(m => m.id === e.id ? { ...m, status: e.status } : m)
        );
      }
    });
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? 'bg-indigo-50' : 'hover:bg-slate-50/80'
                )}
              >
                {active && (
