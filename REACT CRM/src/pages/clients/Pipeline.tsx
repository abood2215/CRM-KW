  ...
  return (
    <div className="h-full flex flex-col font-cairo">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 px-1">
        <div>
          <h1 className="text-2xl font-black text-slate-800">تتبع العملاء (Pipeline)</h1>
          <p className="text-slate-500 font-medium mt-1">نظام إدارة وتحويل العملاء المحتملين لمركز مطمئنة.</p>
        </div>
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex gap-6 overflow-x-auto pb-8 scrollbar-thin h-[calc(100vh-230px)] min-h-[500px]">
          {STAGES.map((stage) => (
            <div key={stage.id} className="flex-shrink-0 w-[300px] flex flex-col group">
              <div className={cn(
                "flex items-center justify-between px-4 py-3 border-b-2 mb-4 rounded-xl shadow-sm",
                stage.color
              )}>
                <div className="flex items-center gap-2.5">
                  <span className="text-xs font-black uppercase tracking-widest">{stage.title}</span>
                  <span className="bg-white/50 text-[10px] font-bold px-2 py-0.5 rounded-full min-w-5 text-center">
              ...
              <Droppable droppableId={stage.id}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={cn(
                      "flex-1 rounded-2xl transition-all p-2 space-y-4",
                        ...
                        <Draggable key={client.id} draggableId={client.id.toString()} index={index}>
                          {(provided, snapshot) => (
                            <motion.div
                              ...
                              className={cn(
                                "bg-white p-5 rounded-2xl border border-slate-100 shadow-sm transition-all hover:shadow-xl group relative overflow-hidden",
                                snapshot.isDragging ? "shadow-2xl ring-2 ring-indigo-500 rotate-2 scale-105" : "hover:-translate-y-1"
                              )}
                            >
                              <div className="flex items-start justify-between mb-3 relative z-10">
                                <div>
                                  <h4 className="text-sm font-black text-slate-800 line-clamp-1">{client.name}</h4>
                                  <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-wider">{client.source}</p>
                                </div>
                                <button className="p-1 hover:bg-slate-50 rounded-lg text-slate-400 hover:text-indigo-600 transition-colors">
                              ...
                              <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between text-[10px] font-bold text-slate-300 relative z-10">
                                <div className="flex items-center gap-1.5">
                                   <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 border border-white">
                                      <Users size={12} />
                                   </div>
